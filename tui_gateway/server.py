...
def _(rid, params: dict) -> dict:
    ...
    if key == "persona":
        # Return persona identity from PERSONA.md + personality config
        cfg = _load_cfg()
        personality = str((cfg.get("display") or {}).get("personality", "default") or "default")
        persona_path = _hermes_home / "PERSONA.md"
        persona_md = ""
        if persona_path.exists():
            try:
                persona_md = persona_path.read_text(encoding="utf-8").strip()
            except Exception:
                pass
        # Try to extract a display name from PERSONA.md first line
        name = personality
        if persona_md:
            first_line = persona_md.split("\n")[0].strip()
            # Strip markdown headers
            import re
            clean = re.sub(r"^#+\s*", "", first_line).strip()
            if clean and len(clean) < 60:
                name = clean
        # Also check for display.agent_name in config
        display_name = (cfg.get("display") or {}).get("agent_name", "")
        if display_name:
